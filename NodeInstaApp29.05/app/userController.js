...
const sendMail = async (token) => {
  transporter.sendMail({
    ...
    subject: "Acccount Confirmation",
    text: "text",
    html: `<b> Kliknij w poniższy link <br /> <a href="http://localhost:3000/api/user/confirm/${token}"> http://localhost:3000/api/user/confirm/${token} </a>  <br /> W celu potwierdzenia konta <br /> </b>`,
  });
};
...
module.exports = {
  ...
  loginUser: async (userData) => {
    ...
    if (passMatch && foundUser.confirmed) {
      console.log("hasło się zgadza");
      let generatedToken = await createToken(userData);
